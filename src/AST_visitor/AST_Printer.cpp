...
void AST_Printer::visit_var(Variable_AST &expr) {
    ...
    std::cout << ind.get_indent() << "[VAR_EXP] " << raw_to_string(expr.name) << "\n";
}

void AST_Printer::visit_fp(Float_point_AST &expr) {
    Indent ind(cur_indent);
    std::cout << ind.get_indent() << "[FP_EXP] " << expr.val << "\n";
}

void AST_Printer::visit_int(Integer_AST &expr) {
    Indent ind(cur_indent);
    std::cout << ind.get_indent() << "[INT_EXP] " << expr.val << "\n";
}

void AST_Printer::visit_unary(Unary_expr_AST &expr) {
    Indent ind(cur_indent);
    std::cout << ind.get_indent() << "[UNARY_EXP]\n";
    expr.LHS->accept(*this);
}

void AST_Printer::visit_block(Block_AST &expr) {
    Indent ind(cur_indent);
    std::cout << ind.get_indent() << "[BLOCK]\n";
    if (expr.v_expr.empty()) std::cout << ind.get_indent() << "empty code block!\n";
...
void AST_Printer::visit_binary(Binary_expr_AST &expr) {
    Indent ind(cur_indent);
    std::cout << ind.get_indent() << "[LHS_EXP]\n";
    expr.LHS->accept(*this);
    std::cout << ind.get_indent() << "[BIN_OP ] " << names_kind[expr.op] << "\n";
    std::cout << ind.get_indent() << "[RHS_EXP]\n";
    expr.RHS->accept(*this);
}

void AST_Printer::visit_func_proto(Function_proto_AST &expr) {
    Indent ind(cur_indent);
    std::cout << ind.get_indent() << "func name: " << raw_to_string(expr.name) << "(\n";
    for (int i=0; i < expr.args.size(); i++) {
        expr.args[i] -> accept(*this);
        if (i < expr.args.size()-1) std::cout << ind.get_indent() << ", \n";
    }
    std::cout << ind.get_indent() << ")\n";
}

void AST_Printer::visit_func_call(Function_call_AST &expr) {
    Indent ind(cur_indent);
    std::cout << ind.get_indent() << "[FUNC_CALL]\n";
    std::cout << ind.get_indent() << "func : " << raw_to_string(expr.name) <<  " (\n";
    for (int i=0; i < expr.args.size(); i++) {
        expr.args[i] -> accept(*this);
        if (i < expr.args.size()-1) std::cout << ind.get_indent() << ", \n";
    }
...
void AST_Printer::visit_func(Function_AST &expr) {
    Indent ind(cur_indent);
    std::cout << ind.get_indent() << "[FUNC]\n";
    expr.args_with_func_name->accept(*this);
    std::cout << ind.get_indent() << "func body: \n";
    expr.func_body->accept(*this);
    std::cout << ind.get_indent() << "return: \n";
...
void AST_Printer::visit_while(While_AST &expr) {
    Indent ind(cur_indent);
    std::cout << ind.get_indent() << "[While_STMT]\n";
    std::cout << ind.get_indent() << "While ";
    expr.cond->accept(*this);
    std::cout << ind.get_indent() << " is not 0\n";
...
void AST_Printer::visit_def(Define_AST &expr) {
    Indent ind(cur_indent);
    std::cout << ind.get_indent() << "[DEF_STMT]\n";
...
void AST_Printer::visit_assign(Assign_AST &expr) {
    Indent ind(cur_indent);
    std::cout << ind.get_indent() << "[ASSIGN]\n";
    expr.var->accept(*this);
    std::cout << ind.get_indent() << "=\n";
    expr.rhs->accept(*this);
}
